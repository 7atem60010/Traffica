...
if __name__ == '__main__':
    ...
    prs.add_argument("-log", dest="log", type=str, default='cache/2way-single-intersection/log.txt', help="Logger file.\n")
    prs.add_argument("-cache", dest="cache", type=str, default='cache/2way-single-intersection/', help="Cache file to load.\n")
    args = prs.parse_args()
    ...
    for run in range(1, args.runs+1):
        ...
        ql_agents = {ts: QLAgent(starting_state=env.encode(initial_states[ts], ts),
                                 ...
                                 cached=f'{args.cache}/q_table_{ts}') for ts in env.ts_ids}
        ...
        if args.fixed:
            ...
            for agent_id in ql_agents.keys():
                with open(f'{args.cache}/q_table_{agent_id}', 'wb') as f:
                    pickle.dump(ql_agents[agent_id].q_table, f)
                with open(args.log, 'a') as f:
                    print(f"Cached q table for agent {agent_id} at run {run}.", file=f)

            if run % 10 == 0:
                for agent_id in ql_agents.keys():
                    with open(f'{args.cache}/q_table_backup{agent_id}', 'wb') as f:
                        pickle.dump(ql_agents[agent_id].q_table, f)
                    with open(args.log, 'a') as f:
                        print(f"Saved backup for agent {agent_id} at run {run}.", file=f)
