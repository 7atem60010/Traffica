...
class TrafficSignal:
    ...
    def get_emission_time_per_lane(self):
        emissions_per_lane = []
        for lane in self.lanes:
            veh_list = traci.lane.getLastStepVehicleIDs(lane)
            lane_emissions = 0.0
            for veh in veh_list:
                veh_lane = traci.vehicle.getLaneID(veh)
                ems = traci.vehicle.getAccumulatedWaitingTime(veh)
                lane_emissions += self.env.vehicles[veh][veh_lane]
            emissions_per_lane.append(lane_emissions)
        return emissions_per_lane
